  ...
  <NuxtLayout name="slideproduct">
    <ClientOnly>
      ...
      >
        ...
        >
        <img src="/svg/catSlider.svg" alt="">
          <h1 class="pr-2 dark:text-white">محصولات گربه</h1>
          <SlideProductNextPrevBtn />
        </div>
        <SwiperSlide v-for="catProduct in catCategor" :key="catProduct.id">
          <figure
          ...
          >
          <NuxtLink target="_blank" :to="`/products/${catProduct.slug}`">
              <img
              class="transition-transform w-full h-full"
              v-img="`${apiBase}${catProduct.src}`"
              ...
              >
              <svg
              @click="emit('addToCart', catProduct)"
              ...
              />
                ...
                <svg
                  v-if="catProduct.favorite_id in props.favored"
                  @click="emit('favor', catProduct.favorite_id)"
                  class="text-red-700"
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
...
const catCategor = data.value.filter(c => c.category === "catProduct")
