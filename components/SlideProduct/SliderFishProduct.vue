  ...
  <NuxtLayout name="slideproduct">
    <ClientOnly>
      ...
      >
        ...
        >
        <img src="/svg/fishSlider.svg" alt="">
          <h1 class="pr-2 dark:text-white">محصولات ماهی</h1>
          <SlideProductNextPrevBtn />
        </div>
        <SwiperSlide v-for="fishProduct in fishCategor" :key="fishProduct.id">
          <figure
          ...
          >
          <NuxtLink target="_blank" :to="`/products/${fishProduct.slug}`">
              <img
                class="transition-transform w-full h-full"
                v-img="`${apiBase}${fishProduct.src}`"
              ...
              >
                <svg
                  @click="emit('addToCart', fishProduct)"
                ...
                <svg
                  v-if="fishProduct.favorite_id in props.favored"
                  @click="emit('favor', fishProduct.favorite_id)"
                  class="text-red-700"
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
...
const fishCategor = data.value.filter(c => c.category === "fishProduct")
