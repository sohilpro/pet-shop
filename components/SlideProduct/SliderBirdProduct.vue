  ...
  <NuxtLayout name="slideproduct">
    <ClientOnly>
      ...
      >
        ...
        >
          <img src="/svg/birdSlider.svg" alt="..." />
          <h1 class="pr-2 dark:text-white">محصولات پرنده</h1>
          <SlideProductNextPrevBtn />
        </div>
        <SwiperSlide v-for="birdProduct in BirdCategor" :key="birdProduct.id">
            <figure
            ...
            >
              <NuxtLink target="_blank" :to="`/products/${birdProduct.slug}`">
                <img
                class="transition-transform w-full h-full"
                v-img="`${apiBase}${birdProduct.src}`"
              ...
              >
                <svg
                  @click="emit('addToCart', birdProduct)"
                ...
                <svg
                  v-if="birdProduct.favorite_id in props.favored"
                  @click="emit('favor', birdProduct.favorite_id)"
                  class="text-red-700"
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
...
const { data: birdProducts } = await useFetch(`${apiBase}/products/`);

const BirdCategor = birdProducts.value.filter(c => c.category === "birdProduct")
