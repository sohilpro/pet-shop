<template>
  <Body class="bg-custom h-[760px] md:overflow-hidden w-screen md:h-screen">
    <div class="center m-auto mt-7">
      <AuthHead />
      <form @submit.prevent="register" class="login">
        ...
        <button v-else type="submit" class="login-button">ثبت نام</button>
      </form>
      <AuthScoial />
...
useHead({
  title: toTitleCase("Register page"),
});
...
const register = async () => {
  ...
  if (
    ...
    !passwordPattern.test(formData.password2)
  ) {
  ...
  try {
    ...
    if (
      ...
      toast.error("این نام کاربری یا ایمیل وجود دارد.");
      return;
    }
    toast.error("اوه متاسفانه مشکلی پیش آمده!");
  } finally {
    loading.value = false;
  }
};
